...
export default {
  ...
  methods:{
    ...
    query:function (){
      this.axios.post('/api/user/querycustmess', JSON.stringify({"name":window.sessionStorage.getItem('loginname')}),
      ).then(res => {//true
      ...
      this.axios.post('/api/cust/querylog', JSON.stringify({"name":this.name}),
      ).then(res => {//true
        this.log = res.data[0]
    ...
    edit:function (){
      ...
      if (this.read) {
        ...
        if(this.$v.mess.$invalid||this.$v.mess.$error){
          ...
          var mess =  this.mess
          delete  mess.addr
          this.axios.post('/api/user/savecustmess', JSON.stringify(mess)).then(res => {//true
    ...
    change_Pass:function (){
      if(this.$v.$invalid||this.$v.$error){
        ...
        this.axios.post('/api/cust/changecustpass', JSON.stringify(this.pass),
        ).then(res => {//true
          this.savemess = res.data["mess"];
          this.show=true;
          this.query();
